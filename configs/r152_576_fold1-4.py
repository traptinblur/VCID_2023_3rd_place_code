...
class CFG:
    ...
    device = "cuda"
    # ============== model cfg =============
    model_name = "adaptive_silu_r152"

    ema = True
    model_ema_decay = 0.9994
    positive_only = False
    clean = True
    normalization = True
    loss_weights = [3, 1]
    p_mixup = 0.4
    p_switch = 0.5  # switch mixup to cutmix
    rotate_limit = 180
    ...
    # ============== training cfg =============
    size = 576  # crop size
    tile_size = 576  # same as size
    stride = tile_size // 4

    batch_size = 2
    accum_iter = 2  # 1, maintain bs to 16
    use_amp = True
    epochs = 20
    ...
    # ============== fold =============
    folds = [1, 4]  # specify which fold needs experiment on
