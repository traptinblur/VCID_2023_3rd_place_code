...
class CFG:
    ...
    device = "cuda"
    # ============== model cfg =============
    model_name = "adaptive_silu"#"adaptive_silu_r152"#"hybrid_silu_r152""hybrid_silu"#"adaptive_silu_r152"

    exp_name = f'3090_{model_name}_exp053'
    ...
    batch_size = 4
    accum_iter = 1  # 1, maintain bs to 16
    use_amp = True

    epoch_dict = {  # fold specified training epoch
        "1": 25,
        "2": 25, 
        "3": 25,
        "4": 25,
    ...
    verbose = (f"\n{len(slice_idcs)} slices {slice_idcs}, tr slices {tr_slices} with idices {tr_idx}\n"
               ...
               f"model {model_name}\n"
               f"bs {batch_size}, acctual bs {batch_size*accum_iter}, epochs {epoch_dict}\n"
...
def main(args):
    ...
    for args.valid_id in args.folds:
        ...
        args.epochs = args.epoch_dict[str(args.valid_id)]
